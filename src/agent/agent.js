...
export class Agent {
	...
	async handleMessage(source, message, max_responses = null) {
		...
		if (!self_prompt && !from_other_bot) {
			// from user, check for forced commands
		...
		const checkInterrupt = () =>
			this.self_prompter.shouldInterrupt(self_prompt) || this.shut_up || convoManager.responseScheduledFor(source);
		...
		for (let i = 0; i < max_responses; i++) {
			...
			if (command_name) {
				...
				if (settings.show_command_syntax === "full") {
					...
					// no command at all
					let pre_message = res.substring(0, res.indexOf(command_name)).trim();
					if (pre_message.trim().length > 0) this.routeResponse(source, pre_message);
				}
