...
export const actionsList = [
	...
	},
	{
		...
		perform: async function (agent, mode_name, on) {
			...
			if (!modes.exists(mode_name)) return `Mode ${mode_name} does not exist.` + modes.getDocs();
			if (modes.isOn(mode_name) === on) return `Mode ${mode_name} is already ${on ? "on" : "off"}.`;
	...
	},
	{
		...
		perform: async function (agent, player_name, message) {
			if (!convoManager.isOtherAgent(player_name)) return player_name + " is not a bot, cannot start conversation.";
			if (convoManager.inConversation() && !convoManager.inConversation(player_name))
				convoManager.forceEndCurrentConversation();
			else if (convoManager.inConversation(player_name))
				agent.history.add(
					"system",
					"You are already in conversation with " + player_name + ". Don't use this command to talk to them.",
				);
	...
	},
	{
		...
		perform: async function (agent, player_name) {
			if (!convoManager.inConversation(player_name)) return `Not in conversation with ${player_name}.`;
