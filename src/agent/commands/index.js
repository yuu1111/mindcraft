...
export function parseCommandMessage(message) {
	...
	for (let i = 0; i < args.length; i++) {
		...
		switch (param.type) {
			...
			case "ItemName":
				if (arg.endsWith("plank") || arg.endsWith("seed")) arg += "s"; // add 's' to for common mistakes like "oak_plank" or "wheat_seed"
			case "string":
				break;
			default:
				throw new Error(`Command '${commandName}' parameter '${paramNames[i]}' has an unknown type: ${param.type}`);
		}
		if (arg === null || Number.isNaN(arg)) return `Error: Param '${paramNames[i]}' must be of type ${param.type}.`;
