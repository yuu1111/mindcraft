...
class Conversation {
	...
	reset() {
		...
		if (full_message.message.trim().length > 0) agent.history.add(this.name, full_message.message);
		// add the full queued messages to history, but don't respond

		if (agent.last_sender === this.name) agent.last_sender = null;
	}
...
class ConversationManager {
	...
	_getConvo(name) {
		if (!this.convos[name]) this.convos[name] = new Conversation(name);
		...
		this.connection_monitor = setInterval(() => {
			...
			if (this.awaiting_response && agent.isIdle()) {
				...
				if (wait_time > this.wait_time_limit) {
					agent.handleMessage(
						"system",
						`${convo_partner} hasn't responded in ${this.wait_time_limit / 1000} seconds, respond with a message to them or your own action.`,
					);
					wait_time = 0;
	...
	sendToBot(send_to, message, start = false, open_chat = true) {
		...
		if (settings.chat_bot_messages && open_chat) agent.openChat(`(To ${send_to}) ${message}`);

		if (convo.ignore_until_start) return;
		convo.active = true;
	...
	async receiveFromBot(sender, received) {
		...
		if (convo.ignore_until_start && !received.start) return;

		// check if any convo is active besides the sender
		if (this.inConversation() && !this.inConversation(sender)) {
			this.sendToBot(
				sender,
				`I'm talking to someone else, try again later. !endConversation("${sender}")`,
				false,
				false,
			);
	...
	responseScheduledFor(sender) {
		if (!this.isOtherAgent(sender) || !this.inConversation(sender)) return false;
		const convo = this._getConvo(sender);
	...
	inConversation(other_agent = null) {
		if (other_agent) return this.convos[other_agent]?.active;
...
async function _scheduleProcessInMessage(sender, received, convo) {
	if (convo.inMessageTimer) clearTimeout(convo.inMessageTimer);
	...
	if (!agent.isIdle() && otherAgentBusy) {
		...
		if (canTalkOver) scheduleResponse(fastDelay);
		...
		if (canTalkOver) {
			...
			if (shouldRespond) scheduleResponse(fastDelay);
		}
	} else {
		// neither are busy
		scheduleResponse(fastDelay);
...
function _handleFullInMessage(sender, received) {
	...
	} else if (received.start) agent.shut_up = false;
