...
async function equipHighestAttack(bot) {
	let weapons = bot.inventory
		.items()
		.filter((item) => item.name.includes("sword") || (item.name.includes("axe") && !item.name.includes("pickaxe")));
	if (weapons.length === 0)
...
export async function pickupNearbyItems(bot) {
	...
	const getNearestItem = (bot) =>
		bot.nearestEntity((entity) => entity.name === "item" && bot.entity.position.distanceTo(entity.position) < distance);
...
export async function placeBlock(bot, blockType, x, y, z, placeOn = "bottom", dontCheat = false) {
	...
	if (bot.modes.isOn("cheat") && !dontCheat) {
		...
		bot.chat(
			"/setblock " + Math.floor(x) + " " + Math.floor(y + 1) + " " + Math.floor(z) + " " + blockType + "[half=upper]",
		);
		...
		bot.chat(
			"/setblock " + Math.floor(x) + " " + Math.floor(y) + " " + Math.floor(z - 1) + " " + blockType + "[part=head]",
		);
		...
		if (stuck_time > 1200) {
			...
			for (let position of positions) {
				...
				if (
					...
					(block.name.includes("door") || block.name.includes("fence_gate") || block.name.includes("trapdoor"))
				) {
