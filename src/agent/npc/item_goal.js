...
class ItemWrapper {
	...
	createChildren() {
		...
		if (block_sources.length > 0 && this.name !== "torch" && !this.name.includes("bed")) {
...
export class ItemGoal {
	...
	async executeNext(item_name, item_quantity = 1) {
		if (this.nodes[item_name] === undefined) this.nodes[item_name] = new ItemWrapper(this, null, item_name);
