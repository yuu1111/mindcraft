 ...
 */
export function proceduralGeneration(
	m = 20,
	n = 20,
	p = 20,
	rooms = 8,
		...
		for (let x = 0; x < m; x++) {
			for (let y = 0; y < n; y++) {
				if (
					...
					x === 0 ||
					x === m - 1 || // Front and back faces
					y === 0 ||
					y === n - 1 // Left and right faces
				) {
					matrix[z][x][y] = "stone";
				}
			}
	...
	function addWindowsAsSquares(matrix, x, y, z, newLength, newWidth, newDepth, material) {
		...
		function isInBounds(z, x, y) {
			return z >= 0 && z < matrixDepth && x >= 0 && x < matrixLength && y >= 0 && y < matrixWidth;
		}
		...
		if (Math.random() < 0.8) {
			...
			for (let dx = 0; dx <= windowX; dx++) {
				for (let dy = 0; dy <= windowY; dy++) {
					...
					if (
						isInBounds(frontZ, centerX + dx, centerY + dy) &&
						matrix[frontZ][centerX + dx][centerY + dy] === material
					) {
						matrix[frontZ][centerX + dx][centerY + dy] = "glass";
					}
					if (isInBounds(backZ, centerX + dx, centerY + dy) && matrix[backZ][centerX + dx][centerY + dy] === material) {
		...
		if (Math.random() < 0.8) {
			...
			for (let dz = 0; dz <= windowZ; dz++) {
				for (let dy = 0; dy <= windowY; dy++) {
					...
					if (isInBounds(centerZ + dz, leftX, centerY + dy) && matrix[centerZ + dz][leftX][centerY + dy] === material) {
						matrix[centerZ + dz][leftX][centerY + dy] = "glass";
					}
					if (
						isInBounds(centerZ + dz, rightX, centerY + dy) &&
		...
		if (Math.random() < 0.8) {
			...
			for (let dx = 0; dx <= windowX; dx++) {
				for (let dz = 0; dz <= windowZ; dz++) {
					...
					if (
						isInBounds(centerZ + dz, centerX + dx, bottomY) &&
						matrix[centerZ + dz][centerX + dx][bottomY] === material
					) {
						matrix[centerZ + dz][centerX + dx][bottomY] = "glass";
					}
					if (isInBounds(centerZ + dz, centerX + dx, topY) && matrix[centerZ + dz][centerX + dx][topY] === material) {
				...
				switch (direction) {
					...
					case "forward":
						...
						if (validateAndBuildBorder(matrix, newX, newY, newZ, newLength, newWidth, newDepth, m, n, p, material)) {
							...
							addDoor(
								matrix,
								lastRoom.x + Math.floor(lastRoom.length / 2),
				...
				row
					.map((cell) => {
						switch (cell) {
							...
							case "air":
								return "."; // Open space
							case "dark_oak_door[half=upper, hinge=left]":
								return "D";
							case "dark_oak_door[half=lower, hinge=left]":
								return "D";
							case "oak_stairs[facing=north]":
								return "S"; // Stairs
							case "oak_stairs[facing=east]":
								return "S"; // Stairs
							case "oak_stairs[facing=south]":
								return "S"; // Stairs
							case "oak_stairs[facing=west]":
								return "S"; // Stairs
							case "glass":
								return "W";

							default:
								return "?"; // Unknown or unmarked space
						}
					})
					.join(" "),
			);
...
function matrixToBlueprint(matrix, startCoord) {
	...
	return {
		levels: matrix.map((level, levelIndex) => ({
			level: levelIndex,
			coordinates: [startX, startY + levelIndex, startZ],
